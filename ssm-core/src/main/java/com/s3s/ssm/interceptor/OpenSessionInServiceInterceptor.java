...
public class OpenSessionInServiceInterceptor implements MethodInterceptor {
   ...
   * The logger.
   */
  private static Log s_logger = LogFactory.getLog(OpenSessionInServiceInterceptor.class);
  ...
  /**
   * Get a Session for the SessionFactory that this filter uses. Note that this just applies in single session
   * mode!
   * <p>
   * The default implementation delegates to the <code>SessionFactoryUtils.getSession</code> method and sets
   * the <code>Session</code>'s flush mode to "NEVER".
   * <p>
   ...
   * <p>
   * Can be overridden in subclasses, e.g. for flushing the Session before closing it. See class-level javadoc
   * for a discussion of flush handling. Note that you should also override getSession accordingly, to set the
