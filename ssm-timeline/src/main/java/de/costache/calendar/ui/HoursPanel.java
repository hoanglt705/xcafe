...
public class HoursPanel extends JPanel {
  ...
  public void paint(Graphics g) {
    ...
    int workingHoursEnd = calendar.getConfig().getWorkingHoursEnd();
    int workingHoursStart = calendar.getConfig().getWorkingHoursStart();
    ...
    for (int i = workingHoursStart; i < workingHoursEnd; i++) {
      ...
      g.setColor(calendar.getConfig().getLineColor());
      g.drawLine(0, y, getWidth(), y);
      g.setColor(calendar.getConfig().getMiddleLineColor());
      g.drawLine(0, y - 30, getWidth(), y - 30);
    }
    ((Graphics2D) g).setStroke(new BasicStroke(2f));
    g.setColor(calendar.getConfig().getTodayHeaderBackgroundColor());

    int yNow = CalendarUtil.secondsToPixels(new Date(), calendar.getConfig().getHeigh());
