...
public class DayHeaderPanel extends JPanel {
  ...
  public void paint(final Graphics g) {
    ...
    JCalendar calendar = owner.getCalendar();
    Config config = calendar.getConfig();

    final boolean isToday = CalendarUtil.isToday(owner.getDate());
    final Color bgColor = isToday ? config.getTodayHeaderBackgroundColor() : config
            .getDayHeaderBackgroundColor();
    final Color fgColor = isToday ? config.getTodayHeaderForegroundColor() : config
            .getDayHeaderForegroundColor();
